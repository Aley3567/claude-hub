...
- 入场和每次交互后允许一段有上限的低帧率呼吸窗口，默认 8 秒；窗口结束后恢复零定时唤醒的阻塞等待。支持 `CLAUDE1_BREATH_SECONDS=0` 只保留入场动画、`CLAUDE1_NO_ANIMATION=1` 关闭全部动画，非 TTY 和小终端自动降级。
- Provider 前三项保持 CC Switch 顺序和数字编号稳定；其余项按带时间衰减的使用频率排序，最近使用仍决定默认光标和“最近”标记。
...
- 动画不吞键、呼吸窗口有上限并最终回到零唤醒；
- 数字直达、前三项固定、其余项 frecency 排序、滚动列表、别名直达；
