...
## 实施后（2026-08-16，L1–L6 全部落地）

同机基准：命中路径（稳态每请求成本）p50 0.02ms / p95 0.023ms（n=200）；
revision 变化时的 refresh p50 43.7ms / p95 143.2ms（n=25，与原路径相同，预期）。
六层各自一个 commit：`20f9db4`（L1）、`2ef0552`（L2）、`96c21f9`（L3）、
`e945ed8`（L4）、`8b765a3`（L5）。全套件 605 tests + 1440 subtests 全绿。
