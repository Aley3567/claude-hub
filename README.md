...
大 Logo 只在进入时流动并呼吸，最长 `240ms`；随后自动收起，切换到以
“欢迎使用 claude1”开头的紧凑选择器。选择器阻塞等待按键，没有后台动画和
定时唤醒；按 `q` 或 `Esc` 退出后清除界面，只留下简短的 Bye 欢迎语。需要
完全跳过开场动画时使用 `CLAUDE1_NO_ANIMATION=1`。
